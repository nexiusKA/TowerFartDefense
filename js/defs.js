// ─────────────────────────────────────────────────────────────────────────────
// TOWER & ENEMY DEFINITIONS
// ─────────────────────────────────────────────────────────────────────────────

const TOWER_DEFS = {
  stinker: {
    name: 'Stench Lord', cost: 75, sellRatio: 0.6,
    color: '#27ae60', color2: '#2ecc71',
    range: 75, damage: 6, fireRate: 1200,
    sound: 'stinker',
    upgradeCost: 60,
    upgradeDmg: 4, upgradeRange: 10, upgradeFireRate: -150,
    desc: 'Reeks of justice · Slows + poisons on hit',
    effect: 'slow',
  },
  blaster: {
    name: 'Fart Bomb', cost: 100, sellRatio: 0.6,
    color: '#e67e22', color2: '#f39c12',
    range: 57, damage: 28, fireRate: 2000,
    sound: 'blaster',
    upgradeCost: 80,
    upgradeDmg: 14, upgradeRange: 7, upgradeFireRate: -300,
    desc: 'Crop-duster special · AoE stink explosion',
    effect: 'aoe',
  },
  honker: {
    name: 'Toilet', cost: 125, sellRatio: 0.6,
    color: '#8B4513', color2: '#a0522d',
    range: 100, damage: 16, fireRate: 850,
    sound: 'honker',
    upgradeCost: 100,
    upgradeDmg: 9, upgradeRange: 13, upgradeFireRate: -100,
    desc: 'Fires poop missiles · Pierces enemies · Sees stealth',
    effect: 'pierce',
  },
  fogger: {
    name: 'Sewer Gas', cost: 110, sellRatio: 0.6,
    color: '#5588cc', color2: '#88bbff',
    range: 65, damage: 9, fireRate: 900,
    sound: 'fogger',
    upgradeCost: 85,
    upgradeDmg: 5, upgradeRange: 10, upgradeFireRate: -120,
    desc: 'Rancid cloud bomb · Hits all in the stink zone',
    effect: 'cloud',
  },
  superblast: {
    name: 'Toilet Brush', cost: 200, sellRatio: 0.6,
    color: '#00ccff', color2: '#88eeff',
    range: 115, damage: 50, fireRate: 3000,
    sound: 'superblast',
    upgradeCost: 120,
    upgradeDmg: 20, upgradeRange: 15, upgradeFireRate: -400,
    desc: 'Maximum stink deployed · Wipes ALL enemies in range',
    effect: 'superblast',
  },
  spraycan: {
    name: 'Spray Can', cost: 90, sellRatio: 0.6,
    color: '#6b3a1e', color2: '#8B4513',
    range: 62, damage: 8, fireRate: 800,
    sound: 'spraycan',
    upgradeCost: 70,
    upgradeDmg: 4, upgradeRange: 8, upgradeFireRate: -100,
    desc: 'Brown mist barrage · Rapid-fire single target',
    effect: 'spray',
  },
  toothpaste: {
    name: 'Toothpaste', cost: 130, sellRatio: 0.6,
    color: '#7a5c10', color2: '#c8a020',
    range: 92, damage: 14, fireRate: 1100,
    sound: 'toothpaste',
    upgradeCost: 100,
    upgradeDmg: 8, upgradeRange: 11, upgradeFireRate: -130,
    desc: 'Brown paste precision · Pierces + sees stealth',
    effect: 'paste',
  },
};

const ENEMY_DEFS = {
  basic:   { name: 'Stinky Mole',        hp:  80, speed: 1.0, reward: 10, color: '#8B4513', size: 15, armor: 0 },
  fast:    { name: 'Gas-Propelled Mole', hp:  45, speed: 2.3, reward:  8, color: '#DAA520', size: 12, armor: 0 },
  tank:    { name: 'Bog Mole',           hp: 300, speed: 0.6, reward: 25, color: '#778899', size: 18, armor: 3 },
  stealth: { name: 'Silent But Deadly',  hp:  65, speed: 1.6, reward: 18, color: '#9b59b6', size: 13, armor: 0 },
  swarm:   { name: 'Fart Gnat',          hp:  22, speed: 3.2, reward:  4, color: '#e74c3c', size:  9, armor: 0 },
  boss:    { name: 'Grand Stench Lord',  hp: 700, speed: 0.8, reward: 90, color: '#c0392b', size: 22, armor: 5 },
};
